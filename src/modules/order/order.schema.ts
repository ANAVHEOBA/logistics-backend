...
const orderSchema = new Schema(
  {
    ...
    deliveryAddress: {
      ...
      validate: {
        ...
        message:
          "Delivery address must be either a valid address ID or a complete manual address",
      },
    },
    packageSize: {
      type: String,
...
orderSchema.pre("validate", async function (this: IOrderDocument, next) {
  if (this.isNew) {
    ...
    this.estimatedWeight = calculateEstimatedWeight(
      this.packageSize as PackageSize,
    );
...
orderSchema.post(
  "save",
  async function (doc: IOrderDocument, next): Promise<void> {
    // Notify the admin
